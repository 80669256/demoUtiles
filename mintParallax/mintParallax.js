(function(window, document, $) {
    ...
    mintParallax.prototype.mouseMoveHandler = function(ev) {
    	var oEvent = ev || event;
        var self = this;
        var x = (oEvent.pageX - this.cX) / this.cHalfWidth * self.xRange * (self.invertX ? -1 : 1);
        var y = (oEvent.pageY - this.cY) / this.cHalfHeight * self.yRange * (self.invertY ? -1 : 1);

        this.layers.forEach(function(layer) {
            layer.vx = x;
            layer.vy = y;
            layer.style.transform = "translate3d(" + layer.vx * layer.depth + "px," + layer.vy * layer.depth + "px,0)";
            layer.style[transformName] = "translate3d(" + layer.vx * layer.depth + "px," + layer.vy * layer.depth + "px,0)";
        });
    };

    mintParallax.prototype.devOrientHandler = function(ev) {
    	var oEvent = ev || event;
        this.orientationData = {
            x: oEvent.beta,
            y: oEvent.gamma > 180 ? oEvent.gamma - 360 : oEvent.gamma
        };
    };
